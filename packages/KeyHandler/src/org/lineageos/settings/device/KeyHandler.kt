...
class KeyHandler(private val context: Context) : DeviceKeyHandler {
        ...
        when (previousState) {
            ...
            is Previous.DndRule -> {
                ...
                if (previousRuleId.isNotEmpty()) {
                    setZenRule(previousRuleId, Condition.STATE_FALSE, "Tri-State Key position")
                }
            }
        }
    }
    ...
    private fun handleDndAction(dndKey: String) {
        ...
        setZenRule(selectedZenRuleId, Condition.STATE_TRUE, "Tri-State Key position")
    ...
    private fun setZenRule(ruleId: String, state: Int, summary: String) {
        while (notificationManager.getAutomaticZenRuleState(ruleId) != state) {
