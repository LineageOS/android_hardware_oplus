...
int main() {
    ...
    const std::string instance1 = std::string(HighTouchPollingRate::descriptor) + "/default";
    status = AServiceManager_addService(htpr->asBinder().get(), instance1.c_str());
    if (status != STATUS_OK) {
    ...
    status = AServiceManager_addService(tg->asBinder().get(), instance2.c_str());
    if (status != STATUS_OK) {
