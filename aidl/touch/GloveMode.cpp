...
ndk::ScopedAStatus GloveMode::getEnabled(bool* _aidl_return) {
    ...
    if (mOplusTouch) {
        mOplusTouch->touchReadNodeFile(DEFAULT_TP_IC_ID, GLOVE_MODE_ENABLE_NODE, &value);
...
ndk::ScopedAStatus GloveMode::setEnabled(bool enable) {
    if (mOplusTouch) {
        mOplusTouch->touchWriteNodeFileOneWay(DEFAULT_TP_IC_ID, GLOVE_MODE_ENABLE_NODE,
                                              enable ? "1" : "0");
