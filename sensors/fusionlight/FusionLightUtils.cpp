...
bool ParseConfig(std::istream& stream, FusionConfig& config, std::string& error) {
    ...
    if (!common.isObject() || !crop.isObject() || !resolution.isObject() ||
        ...
        !ReadRequiredInt(common, "SFScreenshotCalDelay", config.sf_screenshot_cal_delay_ns) ||
        !ReadRequiredInt(common, "SFScreenshotCalDelay90", config.sf_screenshot_cal_delay_90_ns) ||
        !ReadRequiredInt(common, "SFScreenshotCalDelay60", config.sf_screenshot_cal_delay_60_ns) ||
        !ReadRequiredInt(common, "SFScreenshotPeriod", config.sf_screenshot_period_millis) ||
        !ReadRequiredNumber(common, "SFScreenshotMatchRatioThreshold",
                            config.sf_screenshot_match_ratio_threshold)) {
    ...
    if (levels == 0 || config.linearity.size() != levels || config.leakage.size() != levels ||
        ...
        config.sf_screenshot_period_millis <= 0 ||
        config.sf_screenshot_match_ratio_threshold <= 0.0 ||
        config.sf_screenshot_match_ratio_threshold > 1.0) {
