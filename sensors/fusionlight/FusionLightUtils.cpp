...
bool ParseConfig(std::istream& stream, FusionConfig& config, std::string& error) {
    ...
    if (!common.isObject() || !crop.isObject() || !resolution.isObject() ||
        ...
        !ReadRequiredInt(common, "CWBScreenshotCalDelay", config.cwb_screenshot_cal_delay_ns) ||
        ...
        !ReadRequiredInt(common, "CWBScreenshotPeriod", config.cwb_screenshot_period_millis) ||
...
void LoadCwbWeights(FusionConfig& config) {
    ...
    LOG(INFO) << "Loaded " << config.cwb_weights.size() << " CWB weights from " << kCwbWeightsPath;
}
