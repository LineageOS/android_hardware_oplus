...
interface IDmtpClient {
  ...
  oneway void onTransporStatsChanged(in vendor.oplus.hardware.communicationcenter.TransportStats stats);
  oneway void onVirtualCommCapabilityChanged(in boolean[] capability);
  oneway void receiveMessage(int sessionId, in vendor.oplus.hardware.communicationcenter.DmtpMessage message);
  oneway void sentToNetwork(in byte[] data);
  oneway void sentToDev(int sessionId, in vendor.oplus.hardware.communicationcenter.DmtpFrameWrapper frame);
}
