...
class EsimController(private val context: Context) {
    ...
    fun hasSN220Chipset() = SystemProperties.get(NFC_CONFIG_FILE_NAME_PROP).contains("SN220")

    fun setUimPower(state: Int) {
        oplusEsimService?.setUimPower(state)
    }
        ...
        try {
            val seService = SEService(
                ...
                object : SEService.OnConnectedListener {
                    override fun onConnected() = synchronized(connectionMutex) {
                        connected = true
