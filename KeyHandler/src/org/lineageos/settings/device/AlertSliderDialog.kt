...
class AlertSliderDialog(private var context: Context)
    ...
    init {
        ...
        length = if (isLandscape) res.getDimension(R.dimen.alert_slider_dialog_width).toInt()
    ...
    fun setState(position: Int, ringerMode: Int) {
        ...
        frameView.setBackgroundResource(backgroundFor(rotation, position))
    ...
    private fun backgroundFor(rotation: Int, position: Int): Int =
        when (rotation) {
            Surface.ROTATION_90 -> when (position) {
                KeyHandler.POSITION_TOP -> R.drawable.alert_slider_top_90
                KeyHandler.POSITION_MIDDLE -> R.drawable.alert_slider_middle
                KeyHandler.POSITION_BOTTOM -> R.drawable.alert_slider_bottom_90
                else -> R.drawable.alert_slider_middle
            }

            Surface.ROTATION_270 -> when (position) {
                KeyHandler.POSITION_TOP -> R.drawable.alert_slider_top_270
                KeyHandler.POSITION_MIDDLE -> R.drawable.alert_slider_middle
                KeyHandler.POSITION_BOTTOM -> R.drawable.alert_slider_bottom_270
                else -> R.drawable.alert_slider_middle
            }

            else -> when (position) { // ROTATION_0 / ROTATION_180
                KeyHandler.POSITION_TOP -> R.drawable.alert_slider_top
                KeyHandler.POSITION_MIDDLE -> R.drawable.alert_slider_middle
                KeyHandler.POSITION_BOTTOM -> R.drawable.alert_slider_bottom
                else -> R.drawable.alert_slider_middle
            }
        }
