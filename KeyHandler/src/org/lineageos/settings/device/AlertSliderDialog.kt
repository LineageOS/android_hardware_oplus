...
class AlertSliderDialog(context: Context) : Dialog(context, R.style.alert_slider_theme) {
    ...
    fun setState(position: Int, ringerMode: Int, flip: Boolean) {
        window!!.attributes = window.attributes.apply {
            ...
            val f = resources.getFraction(R.fraction.alert_slider_dialog_y, 1, 1)
            val h = resources.getDimension(R.dimen.alert_slider_dialog_height).toInt()
            val hv = h + resources.getDimension(R.dimen.alert_slider_container_padding).toInt() * 2

            x = resources.displayMetrics.widthPixels / 100
            y = ((resources.displayMetrics.heightPixels * f) - (hv * 0.5)).toInt()
