...
import androidx.preference.ListPreference // Import ListPreference
import androidx.preference.Preference // Import base Preference
...
class ButtonSettingsFragment : PreferenceFragmentCompat() {
    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        //    This wires up the controller (action) to the dependent (dnd mode)
    ...
    private fun setupPreferenceLogic(
        ...
        Log.d("ButtonSettingsFragmentA", "DND Entries: L${dndEntries.size} ${dndEntries.joinToString()}")
        Log.d("ButtonSettingsFragmentA", "DND Entry Values: L${dndEntryValues.size} ${dndEntryValues.joinToString()}")
     ...
     */
    private fun updateDndPreferenceVisibility(
        dndPreference: Preference,
        controllerValue: String?
    ) {
        // Show the DND preference ONLY if the selected action is "dnd_mode"
        // (You must ensure "dnd_mode" is the value in your arrays.xml for that option)
        dndPreference.isVisible = (controllerValue == VALUE_DND_MODE_ACTION)
        Log.d("ButtonSettingsFragmentA", "DND Preference ${dndPreference.key} visibility set to ${dndPreference.isVisible}")
        // set the sharedPreferences to the first element in the dndEntryValues if it becomes visible and make that the default for the preference
        if (dndPreference.isVisible) {
            ...
            if (listPref.value == null || listPref.value!!.isEmpty()) {
                ...
                Log.d("ButtonSettingsFragmentA", "DND Preference ${dndPreference.key} value set to $firstValue")
            }
        }
    }

    /**
     * Fetches all available DND modes (built-in and user-created).
     */
    private fun getDndModes(): Pair<List<CharSequence>, List<CharSequence>> {
        ...
        // Add built-in DND modes
        // You can get these strings from your own R.string resources
        //entries.add("Total Silence")
        //entryValues.add(NotificationManager.INTERRUPTION_FILTER_NONE.toString())

        //entries.add("Alarms Only")
        //entryValues.add(NotificationManager.INTERRUPTION_FILTER_ALARMS.toString())

        //entries.add("Priority Only")
        //entryValues.add(NotificationManager.INTERRUPTION_FILTER_PRIORITY.toString())
        ...
        // // Add user-created automatic (Zen) rules
        // notificationManager.automaticZenRules.values.forEach { rule ->
        //     if (rule.isEnabled) {
        //         entries.add(rule.name)
        //         // The "value" for an automatic rule is its ID
        //         entryValues.add(rule.)
        //     }
        // }
        Log.d("ButtonSettingsFragmentA", "Fetched ${entries.size} DND modes")
        Log.d("ButtonSettingsFragmentA", "Entries: ${entries.joinToString()}")
        return Pair(entries, entryValues)
    }

    /**
     * Define the keys for your preferences.
     * These MUST match the android:key attributes in your R.xml.button_panel
     */
    companion object {
