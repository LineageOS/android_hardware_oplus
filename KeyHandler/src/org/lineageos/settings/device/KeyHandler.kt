...
class KeyHandler(context: Context) : DeviceKeyHandler {
    ...
    private fun handleMode(position: Int, vibrate: Boolean) {
        ...
        when (previousState) {
            is Previous.AudioManaged -> {
                Log.d(
                    TAG,
                    "Previous state was AudioManaged: state=${(previousState as Previous.AudioManaged).ringerMode}",
                )
            }
            is Previous.ZenMode -> {
                Log.d(
                    TAG,
                    "Previous state was ZenMode: mode=${(previousState as Previous.ZenMode).mode}",
                )
            }
            is Previous.DndRule -> {
                Log.d(
                    TAG,
                    "Previous state was DndRule: ruleId=${(previousState as Previous.DndRule).ruleId}",
                )
            }
            else -> {
                Log.d(
                    TAG,
                    "No previous state recorded. Initializing to AudioManaged with current ringer mode.",
                )
        ...
        executorService.submit {
            ...
            when (mode) {
                AudioManager.RINGER_MODE_SILENT -> {
                    ...
                    // setZenMode(Settings.Global.ZEN_MODE_OFF) // don't assume user wants to clear
                    // DND when going to silent: let the undoPrevious() handle it
                    ...
                    Log.d(
                        TAG,
                        "(audio mode silent) Set previousState to AudioManaged with mode: $mode",
                    )
                    previousState = Previous.AudioManaged(mode)
                    Log.d(
                        TAG,
                        "(audio mode silent) Updated ringer mode for position $position, previousState=$previousState",
                    )
                }
                AudioManager.RINGER_MODE_VIBRATE,
                AudioManager.RINGER_MODE_NORMAL -> {
                    // clear dnd rule if it is set by us
                    // setZenMode(Settings.Global.ZEN_MODE_OFF) // don't assume user wants to clear
                    // DND when going to vibrate/sound: let the undoPrevious() handle it
                    setAMRingerModeTo(mode)
                    ...
                    Log.d(
                        TAG,
                        "(audio mode not silent) Set previousState to AudioManaged with mode: $mode",
                    )
                    previousState = Previous.AudioManaged(mode)
                    Log.d(
                        TAG,
                        "(audio mode not silent)Updated ringer mode for position $position, previousState=$previousState",
                    )
                }
                ZEN_PRIORITY_ONLY,
                ZEN_TOTAL_SILENCE,
                ZEN_ALARMS_ONLY -> {
                    // audioManager.ringerModeInternal = AudioManager.RINGER_MODE_NORMAL // disabled
                    // because we want the default notification mode (vibrate/sound/silent as set by
                    // the user) to persist when activating a zen mode
                    ...
                    Log.d(
                        TAG,
                        "(zen mode) Updated ringer mode to  for position $position, previousState=$previousState",
                    )
                    previousState = Previous.ZenMode(mode)
                    Log.d(
                        TAG,
                        "(zen mode) Set previousState to ZenMode with mode: ${(previousState as Previous.ZenMode).mode}",
                    )
                }
                ACTION_DND_MODE -> {
                    // --- UPDATED DND MODE HANDLING ---
                    // do not change ringer mode, just activate the selected DND rule - we want to
                    // preserve the user's current ringer mode (vibrate/sound/silent)
                    Log.d(TAG, "Handling custom DND action for key $dndKey ($actionKey)")
                    handleDndAction(dndKey)
                    updateRingerMode(position)
                    Log.d(
                        TAG,
                        "(dnd action) Updated ringer mode for position $position, previousState=$previousState",
                    )
                    previousState = Previous.DndRule(sharedPreferences.getString(dndKey, "") ?: "")
                    Log.d(
                        TAG,
                        "(dnd action) Set previousState to DndRule with ID: ${(previousState as Previous.DndRule).ruleId}",
                    )
                }
            }
    ...
    private fun updateRingerMode(position: Int) {
        ...
        Log.d(
            TAG,
            "(updateRingerMode) SharedPreferences contains keys: " +
                sharedPreferences.all.keys.toString(),
        )
        val ringerMode =
            sharedPreferences
        ...
        Log.d(
            TAG,
            "(updateRingerMode) wasMuted state is now $wasMuted, previousState=$previousState",
        )
    }
    ...
    private fun handleDndAction(dndKey: String) {
        ...
        // audioManager.ringerModeInternal = AudioManager.RINGER_MODE_NORMAL // disabled because we
        // want the default notification mode (vibrate/sound/silent as set by the user) to persist
        // when activating a DND rule
        ...
        Log.d(
            TAG,
            "rule state: " +
                notificationManager.getAutomaticZenRuleState(selectedZenRuleId).toString(),
        )
    }
