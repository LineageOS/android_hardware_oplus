...
static void tfa98xx_stop_feedback(void* adev, uint32_t snd_device) {
    ...
    tfa_dev->usecase_tx = tfa_dev->get_usecase_from_list(tfa_dev->adev, USECASE_AUDIO_SPKR_CALIB_TX);
    if (tfa_dev->usecase_tx) {
        list_remove(&tfa_dev->usecase_tx->list);
        tfa_dev->disable_audio_route(tfa_dev->adev, tfa_dev->usecase_tx);
        free(tfa_dev->usecase_tx);
...
static int amp_module_open(const hw_module_t* module, const char* name, hw_device_t** device) {
    ...
    tfa_dev->audio_primary_lib = dlopen(LIB_AUDIO_HAL, RTLD_NOW);
    if (!tfa_dev->audio_primary_lib) {
        ALOGE("%s:%d: Unable to dlopen %s: %s\n", __func__, __LINE__, LIB_AUDIO_HAL, dlerror());
        free(tfa_dev);
        return -ENODEV;
    }

    tfa_dev->enable_snd_device = (int (*)(struct audio_device*, snd_device_t))dlsym(
            tfa_dev->audio_primary_lib, "enable_snd_device");
    tfa_dev->enable_audio_route = (int (*)(struct audio_device*, struct audio_usecase*))dlsym(
            tfa_dev->audio_primary_lib, "enable_audio_route");
    tfa_dev->disable_snd_device = (int (*)(struct audio_device*, snd_device_t))dlsym(
            tfa_dev->audio_primary_lib, "disable_snd_device");
    tfa_dev->disable_audio_route = (int (*)(struct audio_device*, struct audio_usecase*))dlsym(
            tfa_dev->audio_primary_lib, "disable_audio_route");
    tfa_dev->platform_get_pcm_device_id = (int (*)(audio_usecase_t, int))dlsym(
            tfa_dev->audio_primary_lib, "platform_get_pcm_device_id");
    tfa_dev->get_usecase_from_list =
            (struct audio_usecase * (*)(const struct audio_device*, audio_usecase_t))
                    dlsym(tfa_dev->audio_primary_lib, "get_usecase_from_list");

    if (!tfa_dev->enable_snd_device || !tfa_dev->enable_audio_route ||
        !tfa_dev->disable_snd_device || !tfa_dev->disable_audio_route ||
        !tfa_dev->platform_get_pcm_device_id || !tfa_dev->get_usecase_from_list) {
        ALOGE("%s:%d: Unable to dlsym one or more symbols: %s\n", __func__, __LINE__, dlerror());
        dlclose(tfa_dev->audio_primary_lib);
        free(tfa_dev);
        return -ENODEV;
    }
