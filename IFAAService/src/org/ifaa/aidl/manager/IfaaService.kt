...
class IfaaService : Service() {
    ...
    override fun onBind(intent: Intent) = object : IfaaManagerService.Stub() {
        ...
        override fun startBIOManager(authType: Int) = when (authType) {
            AUTH_TYPE_FINGERPRINT -> {
                val managerIntent = Intent(Settings.ACTION_SECURITY_SETTINGS).apply {
                    addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                }

                applicationContext.startActivity(managerIntent)
        ...
        override fun getExtInfo(authType: Int, keyExtInfo: String): String {
            return when (keyExtInfo) {
        ...
        override fun getEnabled(bioType: Int): Int {
            if (keyguardManager?.isKeyguardSecure != true) {
                Log.e(LOG_TAG, "Security keyguard no set")
                return BIOMETRIC_NOUSE_NOSET_KEYGUARD
            }
            return when (bioType) {
                AUTH_TYPE_FINGERPRINT -> when {
                    fingerprintManager?.isHardwareDetected != true -> BIOMETRIC_NOUSE_SYSTEMLOCKED.also {
                        Log.e(LOG_TAG, "Fingerprint hardware not available!")
                    }

                    fingerprintManager?.enrolledFingerprints.isNullOrEmpty() -> BIOMETRIC_NOUSE_NOT_ENROLLED.also {
        ...
        override fun getIDList(bioType: Int): IntArray? = when (bioType) {
            AUTH_TYPE_FINGERPRINT -> fingerprintManager?.enrolledFingerprints?.map { it.biometricId }
                ?.toIntArray()
                ?.also { Log.w(LOG_TAG, "getIDList: fingerprint list!") } ?: run {
                Log.w(LOG_TAG, "getIDList: no FingerprintManager!")
                null
            }
    ...
    private val defaultDisplayDimension: Point? by lazy {
        val dim1 =
            displayNoVerify?.supportedModes?.maxByOrNull { it.physicalHeight }
                ?.run { Point(physicalWidth, physicalHeight) }
        val dim2 = windowManager?.maximumWindowMetrics?.bounds?.run {
            Point(width(), height())
        }
        dim1 ?: dim2 ?: return@lazy null
        Point(maxOf(dim1!!.x, dim2!!.x), maxOf(dim1.y, dim2.y))
    }

    private val iconDiameter by lazy { SystemProperties.getInt(FP_ICON_SIZE_PROP, 190) }

    // Coordinates of the upper left corner
    private val iconLocation: Point by lazy {
        defaultDisplayDimension?.let {
            val iconLocationBottom = SystemProperties.getInt(FP_ICON_LOCATION_PROP, 278)
            Point(
                (it.x - iconDiameter) / 2, it.y - iconLocationBottom - iconDiameter / 2
            )
        } ?: Point(
            OPLUS_DEFAULT_FINGERPRINT_ICON_LOCATION_X, OPLUS_DEFAULT_FINGERPRINT_ICON_LOCATION_Y
        )
    }
    ...
    private fun initExtString() = run {
        ...
        val scale = { pos: Point ->
            displayDimension ?: defaultDisplayDimension ?: pos
            Point(
    ...
    companion object {
        ...
        private const val OPLUS_DEFAULT_FINGERPRINT_ICON_LOCATION_X = 445
        private const val OPLUS_DEFAULT_FINGERPRINT_ICON_LOCATION_Y = 1967
    }
}
